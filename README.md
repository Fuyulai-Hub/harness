...
**使用方式**：
1. 左侧选择或添加**工作区目录**（自动记住历史目录，可随时切换），在底部输入框描述任务
2. 选择模型、**推理强度**（默认/低/中/高，对应 `reasoning_effort` 参数；推理模型建议搭配较大的 max_tokens）和最大步数，点击 ▶ 运行或直接**回车**发送（Shift+Enter 换行）；任务提交后输入框清空、内容置顶显示，运行期间参数锁定，停止或结束后才能改动
3. 右侧以**步骤卡片**实时展示执行轨迹：每步的 🧠 思考过程、⚡ 执行步骤（工具名 + 格式化 JSON 参数）、👁 观察结果（超 600 字符自动折叠）；点击卡片标题折叠单步，点"收起过程/展开过程"整体折叠，摘要条显示 N 次工具调用 · M 步思考
4. 每个任务自动存入左侧**任务历史**（服务端保留，刷新页面可回放）；点"＋ 新建任务"随时开启新任务，互不干扰
...
**执行链路与增强开关**：Agent 核心是 ReAct 循环（每步 思考→工具调用→观察 反馈回历史）。在此基础上提供三个可选增强，配置在 `agent:` 节，也可在输入栏用"规划/反思"勾选框按次切换：

```yaml
agent:
  planning: true             # 运行前先生成显式执行计划（Plan-and-Execute）
  reflection: true           # 每 N 步或连续报错后自我复盘，结论注入后续步骤
  reflect_every: 4           # 复盘间隔步数
  max_history_messages: 40   # 超过此长度自动压缩历史（长任务记忆压缩）
```

开启规划后轨迹顶部会出现"计划"卡片；反思轮以 🪞 反思区块插入对应步骤；历史压缩在后台静默进行以支撑长任务。
